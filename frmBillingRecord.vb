...
Public Class frmBillingRecord
    ...
    Public Sub Getdata()
        Try
            ...
            con.Open()
            cmd = New SqlCommand("Select Inv_ID, RTRIM(InvoiceNo), InvoiceDate, Customer.ID,RTRIM(Customer.CustomerID),RTRIM(Name),RTRIM(ContactNo), GrandTotal, TotalPaid, Balance, RTRIM(InvoiceInfo.Remarks) from Customer,InvoiceInfo where Customer.ID=InvoiceInfo.CustomerID order by InvoiceDate", con)
    ...
    Private Sub dgw_MouseClick(sender As Object, e As System.Windows.Forms.MouseEventArgs) Handles dgw.MouseClick
        Try
            If dgw.Rows.Count > 0 Then
                ...
                frmBilling.txtCustomerID.Text = dr.Cells(4).Value.ToString()
                frmBilling.txtCID.Text = dr.Cells(3).Value.ToString()
                frmBilling.txtCustomerName.Text = dr.Cells(5).Value.ToString()
                frmBilling.txtContactNo.Text = dr.Cells(6).Value.ToString()
                frmBilling.txtGrandTotal.Text = dr.Cells(7).Value.ToString()
                frmBilling.txtTotalPayment.Text = dr.Cells(8).Value.ToString()
                frmBilling.txtPaymentDue.Text = dr.Cells(9).Value.ToString()
                frmBilling.txtRemarks.Text = dr.Cells(10).Value.ToString()
                ...
                While (rdr.Read() = True)
                    frmBilling.DataGridView1.Rows.Add(rdr(0), rdr(1), rdr(2), rdr(3), rdr(4), rdr(5), rdr(6), rdr(7), rdr(8), rdr(9), rdr(10), rdr(11), rdr(12))
                End While
                con.Close()
                con = New SqlConnection(cs)
                con.Open()
                ...
                If rdr.Read Then
                    frmBilling.txtCustomerType.Text = rdr.GetValue(0)
